...
SystemZTargetLowering::emitMoveStackGuard(MachineInstr &MI,
  ...
  // In the TLS (default) case, AddrReg will contain the thread pointer, so we need to
  // add 40 bytes to get the actual address of the stack guard.
  StringRef GuardType = MBB->getParent()->getFunction().getParent()->getStackProtectorGuard();
  unsigned long Offset = (GuardType == "global") ? 0 : 40;
